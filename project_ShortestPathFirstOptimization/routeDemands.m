...
function[]=routeDemands(start,destination,demandMatrix)
...
if flag==1
   startingNode=i;
	%fprintf('Route %d TO %d PRIMARY\t->\t minCost: %d\t',i,j,minCost);
	while minArr(startingNode,2)~=0
   %	fprintf('%d->',startingNode);
		...
		connectionMatrix(minArr(startingNode,2),startingNode).usedCapacity=connectionMatrix(minArr(startingNode,2),startingNode).usedCapacity + demandMatrix(start,destination)/2;
		startingNode=minArr(startingNode,2);
	end
	%fprintf('%d\n',destinationNode);

	%fprintf('Route %d TO %d SECONDARY\t->\t minCost: %d\t',i,j,minCost);
	startingNode=i;
   ...
   %   fprintf('%d->',startingNode);
      connectionMatrix(startingNode,minArr2(startingNode,2)).usedCapacity=connectionMatrix(startingNode,minArr2(startingNode,2)).usedCapacity + demandMatrix(start,destination)/2;
		connectionMatrix(minArr2(startingNode,2),startingNode).usedCapacity=connectionMatrix(minArr2(startingNode,2),startingNode).usedCapacity + demandMatrix(start,destination)/2;
   ...
   %	fprintf('%d->',startingNode);
		...
		connectionMatrix(minArr(startingNode,2),startingNode).usedCapacity=connectionMatrix(minArr(startingNode,2),startingNode).usedCapacity + demandMatrix(start,destination);
		startingNode=minArr(startingNode,2);
	end
   %fprintf('%d\n\n',destinationNode);
end
